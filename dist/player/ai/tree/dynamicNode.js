...
export default class DynamicNode {
   ...
   * @see [Informaion on the
   *     UCT-formula](https://en.wikipedia.org/wiki/Monte_Carlo_tree_search#Exploration_and_exploitation)
   */
  select(playerNumber) {
    ...
    for (const child of this._children) {
      const exploitation = (child._state.wins / child._state.visits) ||
                           0; // Change `NaN` to 0 (0 wins / 0 visits).
      let exploration = Math.sqrt(2) * Math.sqrt(Math.log(this._state.visits) /
                                                 child._state.visits);
      exploration = isNaN(exploration)
                        ? Infinity
                        : exploration; // Change `NaN` to `Infinity` (log(0
                                       // parent visits)).
  ...
  sortChildren() {
    ...
    const mostVisisted =
        copy.sort((x, y) => (y._state.wins / y._state.visits || 0) -
                            (x._state.wins / x._state.visits || 0));
    return mostVisisted;
  }
  getWinRate() {
    let wins = 0;
  ...
  getMostVisitedChild() {
    let child = this._children.reduce((x, y) => {
      return (x._state.visits || 0) > (y._state.visits || 0) ? x : y;
    });
    return child;
  }
}
